...
class Serial(serialposix.PosixSerial):
    ...
    def read(self, size=1):
        """Read size bytes from the serial port. If a timeout is set it may
           ...
           until the requested number of bytes is read. If interrupted by
           a KeyboardInterrupt, before 'size' bytes have been read, the pending
           bytes read will not be lost but will be available to subsequent 
           read() calls."""
